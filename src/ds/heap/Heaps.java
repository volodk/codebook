	...
	private static class ArrayHeap<T extends Comparable<? super T>> implements Heap<T> {
		...
		public T pop() {
			...
			if (hasMoreElements()) {
				...
				int i = head, j, k;
				while (2 * i <= tail
						&& gt(arr[i], min(arr[j = Math.min(2 * i, tail)], arr[k = Math.min(2 * i + 1, tail)]))) {
		...
		private T min(Object obj1, Object obj2) {
			return comparator.compare((T) obj1, (T) obj2) < 0 ? (T) obj1 : (T) obj2;
		}

		private boolean lt(Object obj1, Object obj2) {
			return comparator.compare((T) obj1, (T) obj2) < 0;
		}

		private boolean gt(Object obj1, Object obj2) {
			return comparator.compare((T) obj1, (T) obj2) > 0;
		}
